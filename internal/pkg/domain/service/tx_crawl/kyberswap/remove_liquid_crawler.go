...
type RemoveLiquidCrawlImpl struct{
...
func (c *RemoveLiquidCrawlImpl) Crawl(chainID uint, tokenInfoApi string, start_time uint64) ([]*entity.Transaction, uint64, error){
	skip := 0
	var transactions []*entity.Transaction
	ctx := state.GetContext()
	lastTimestamp := uint64(0)
	for {
		query := fmt.Sprintf(removeLiquidQuery, skip, tx_crawl.GraphFirstLimit, start_time)
